...
fieldCurvatureTEST = 4*(2*(A**2+B**2)-1)
...
mask_spherical = CTF * np.exp(1j * 2 * np.pi * 10* astig) * (np.exp(1j*2*np.pi*spherical))
...
ax[1,2].imshow(np.real(mask_fieldCurvatureTEST))
ax[1,3].imshow(np.real(mask_comaTEST))



ax[0,0].set_title("K-space")
ax[0,1].set_title("Original Image")
ax[0,2].set_title("CTF with spherical + astigmatism")
ax[0,3].set_title("Aberrated Image")
